...
fn parse_data(data: &'static str) -> Vec<Limit> {
    ...
    whole
        ...
        .for_each(|l| {
            ...
            limits_vec.push(Limit{value: end + 1, limittype: LimitType::End})
        });
    ...
    for limit in limits {
        match limit.limittype{
            ...
            LimitType::End=> {
                overlaps -=1;
                if overlaps == 0{
                    if start.value == limit.value{
                        println!("ahahah {} {}", start.value, limit.value);
                    }
                    result += limit.value - start.value;
                    end = limit;
                }
            }
        }
